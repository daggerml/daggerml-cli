...
class TxnContext:
    ...
    def put(self, obj: Any, to: Optional[Ref] = None) -> Ref:
        ...
        if isinstance(obj, Datum) and isinstance(obj.data, Resource) and not isinstance(obj, Deletable):
            self._cleanup_deleted_entry(ref)
        return ref
    ...
    def _cleanup_deleted_entry(self, ref: Ref) -> None:
        """Remove a stale deleted record for the given resource reference.

        Parameters
        ----------
        ref : Ref
            Reference to clean up deleted entries for.
        """
        try:
            deleted_ref = Ref(f"deletable:{ref.id()}")
            if self.txn.exists(deleted_ref):
                self.logger.warning(
                    "Clearing deleted entry %s before reinserting resource %s",
                    deleted_ref,
                    ref,
                )
                self.txn.delete(deleted_ref)
        except Exception as e:
            self.logger.exception(f"cleanup_deleted_entry: ref={ref}")
            raise DmlRepoError(f"Failed to cleanup deleted entry: {e}") from e
    ...
    def get_ctx(self, head: Ref) -> CtxHelper:
        ...
        return CtxHelper(head_obj, commit, tree, dag=dag)
