...
cdef extern from "dml_value.h":
    ...
    ctypedef struct DmlValueResource:
        char *uri
        size_t uri_len
        DmlValue *data
        char *adapter
    ...
    DmlValue *dml_value_new_resource(
        const char *uri,
        size_t uri_len,
        DmlValue *data,
        const char *adapter,
...
cdef DmlValue *py_to_dml_value(object obj):
    ...
    cdef int overflow
    cdef const char *uri_data
    cdef Py_ssize_t uri_len
    cdef const char *adapter_data
    cdef Py_ssize_t adapter_len
    # keep Python-level references to temporary Unicode objects
    cdef object py_str
    cdef object py_to_obj
    cdef object py_uri
    cdef object py_adapter
    ...
    if isinstance(obj, Resource):
        ...
        adapter_len = 0
        py_uri = obj.uri
        uri_data = PyUnicode_AsUTF8AndSize(py_uri, &uri_len)
        if uri_data == NULL:
            return NULL
        data_value = NULL
        if obj.data is not None:
            if not isinstance(obj.data, Ref):
                raise DmlDbInvalidTypeError(
                    f"invalid Resource.data value: {obj.data!r} "
                    "(reason: must be Ref or None)"
                )
                return NULL
            data_value = py_to_dml_value(obj.data)
            if data_value == NULL:
                return NULL
        if obj.adapter is not None:
            if not isinstance(obj.adapter, str):
                dml_value_free(data_value)
                raise DmlDbInvalidTypeError(
                    f"invalid Resource.adapter value: {obj.adapter!r} "
                    "(reason: must be str or None)"
                )
                return NULL
            py_adapter = obj.adapter
            adapter_data = PyUnicode_AsUTF8AndSize(py_adapter, &adapter_len)
            if adapter_data == NULL:
                dml_value_free(data_value)
                return NULL
        result = dml_value_new_resource(
            uri_data,
            <size_t>uri_len,
            data_value,
            adapter_data,
...
cdef object dml_value_to_py(const DmlValue *value):
    ...
    if value.type == DML_VALUE_MAP:
        ...
        return py_dict
    if value.type == DML_VALUE_RESOURCE:
        py_obj = PyUnicode_DecodeUTF8(
            value.as.resource.uri,
            <Py_ssize_t>value.as.resource.uri_len,
            "strict"
        )
        data = None
        if value.as.resource.data != NULL:
            data = dml_value_to_py(value.as.resource.data)
        adapter = None
        if value.as.resource.adapter != NULL:
            py_key = PyUnicode_DecodeUTF8(
                value.as.resource.adapter,
                <Py_ssize_t>value.as.resource.adapter_len,
                "strict"
            )
            adapter = py_key
        return Resource(py_obj, data=data, adapter=adapter)
    raise TypeError("Unsupported msgpack object type")

class DmlError(Exception):
    """
    Base error for native DaggerML bindings.

    Notes
    -----
    This remains for backward compatibility with existing callers.
    """
    pass

class DmlDbError(DmlError):
    """
...
cdef class Resource:
    ...
    uri
        Resource URI.
    data
        Optional Ref for resource data.
    adapter
        Optional adapter name.

    Notes
    -----
    Resources carry external references through the same serialization path as
    ordinary values, so the database can persist and restore them consistently.
    """
    cdef public object uri
    cdef public object data
    cdef public object adapter

    def __init__(self, uri, data=None, adapter=None):
        """
        Initialize a resource wrapper.
        ...
        uri
            Resource URI string.
        data
            Optional `Ref` pointing at resource data.
        adapter
            Optional adapter name used to resolve the resource.

        Raises
        ------
        TypeError
            If arguments have unsupported types.
        """
        if not isinstance(uri, str):
            raise TypeError("uri must be str")
        if data is not None and not isinstance(data, Ref):
            raise TypeError("data must be Ref or None")
        if adapter is not None and not isinstance(adapter, str):
            raise TypeError("adapter must be str or None")
        self.uri = uri
        self.data = data
    ...
    def __repr__(self):
        ...
        -------
        str
            Debug representation showing key fields.
        """
        if self.uri is None:
            return "Resource(<invalid>, None)"
        if self.adapter is None:
            return f"Resource({self.uri}, None)"
        return f"Resource({self.uri}, {self.adapter})"

    def __richcmp__(self, other, int op):
        """
        Compare resources for equality based on their fields.
