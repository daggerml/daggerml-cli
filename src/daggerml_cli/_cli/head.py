...
def execute_head_create(head_ops, args) -> Any:
    ...
    # Get the created head object
    with head_ops._tx(readonly=True) as txn:
        head_obj = txn.get(result)
    # Assume result is Head object with ref and commit
    return {
        "ref": result.to,
        "commit": head_obj.commit.to,
    }
