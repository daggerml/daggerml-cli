...
from daggerml_cli._db import DmlDbInvalidPathError, DmlDbInvalidRefError, Ref
...
class DmlJsonEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, Ref):
            return str(obj)
        if hasattr(obj, "__class__") and obj.__class__.__name__ == "Resource":
            return {
                "__type__": "resource",
                "uri": obj.uri,
                "data": str(obj.data) if obj.data else None,
                "adapter": obj.adapter,
            }
...
def get_repo_path(repo_arg: str | None) -> str:
    ...
    if repo_arg:
        return repo_arg
    env_path = os.environ.get("DML_REPO_PATH")
    if env_path:
        return env_path
    return os.getcwd()
...
def get_ops_object(ops: DmlOps, op_name: str) -> Any:
    ...
    Special case: remote returns the DmlOps instance since remote() requires parameters.
    """
    if op_name == "remote":
        return ops
...
def normalize_error_message(error: Exception, *, command: str | None) -> str:
    ...
    if isinstance(error, (DmlDbInvalidPathError, FileNotFoundError, NotADirectoryError, PermissionError)):
        message = _with_hint(message, "pass --repo PATH or set DML_REPO_PATH")

    # Config errors are generally surfaced via DmlRepoError.
    if isinstance(error, DmlRepoError):
        lowered = message.lower()
        if "s3 bucket" in lowered or "dml_s3_bucket" in lowered or "boto3" in lowered:
...
def execute_command(args) -> None:
    ...
    try:
        ...
        with DmlOps.open(repo_path) as ops:
