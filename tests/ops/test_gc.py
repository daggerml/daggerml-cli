...
from daggerml_cli.types import Commit, Dag, Datum, DmlRepoError, Head, LiteralNode, Tree
from tests.test__db import _gen_ref
from tests.test_types import _head_strategy
    ...
    def test_gc_and_list_orphans(self, temp_bo):
        try:
            with temp_bo._tx(readonly=False) as txn:
                datum_ref = txn.put(Datum(data=42))
            ...
            with temp_bo._tx(readonly=False) as txn:
                new_datum_ref = txn.put(Datum(data="orphan datum"))
            assert ops.list_orphans() == [new_datum_ref]
            stats = ops.gc()
            assert "datum" in stats and stats["datum"] == 1
            assert ops.list_orphans() == []
        finally:
