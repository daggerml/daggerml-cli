...
from daggerml_cli.ops.remote import (
    InvalidManifest,
    InvalidOid,
    InvalidRef,
    MissingCasObject,
...
from tests.conftest import FakeDb, FakeTxn
...
class TestRemoteDescriptor:
    ...
    def test_ensure_descriptor_creates_when_missing(self, db, s3):
        ...
        prefix = os.environ.get("DML_S3_PREFIX", "")
    ...
    def test_ensure_descriptor_validates_existing(self, db, s3):
        """Test that _ensure_remote_descriptor overwrites invalid existing descriptors."""
        bucket = os.environ["DML_S3_BUCKET"]
        prefix = os.environ.get("DML_S3_PREFIX", "")
        descriptor_key = f"{prefix}/dml.json" if prefix else "dml.json"

        # Test invalid descriptor gets overwritten
        invalid_descriptor = {
        ...
        # Create RemoteOps, which should overwrite the invalid descriptor
        RemoteOps(db, s3)

        # Verify descriptor was overwritten with correct content
        response = s3.get_object(Bucket=bucket, Key=descriptor_key)
        descriptor = json.loads(response["Body"].read().decode("utf-8"))
...
class TestRemoteKeyMapping:
    ...
    def test_cas_key_with_prefix(self, remote_ops, monkeypatch):
        ...
        monkeypatch.setenv("DML_S3_PREFIX", "myrepo")
        # Need to recreate remote_ops to pick up the new prefix
        remote_ops_with_prefix = RemoteOps(remote_ops._db, remote_ops.client)
        oid = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
    ...
    def test_ref_key_joining(self, remote_ops):
        ...
        ref_path = "commits/abc123.json"
        expected_key = "test-prefix/refs/commits/abc123.json"
    ...
    def test_ref_key_with_prefix(self, remote_ops, monkeypatch):
        ...
        remote_ops_with_prefix = RemoteOps(remote_ops._db, remote_ops.client)
        ref_path = "commits/abc123.json"
        expected_key = "myrepo/refs/commits/abc123.json"
    ...
    def test_ref_key_rejects_path_traversal(self, remote_ops):
        ...
        with pytest.raises(ValueError, match="Invalid ref path"):
            remote_ops._ref_key("/commits/abc123.json")
        # Test double dot
        with pytest.raises(ValueError, match="Invalid ref path"):
            remote_ops._ref_key("../commits/abc123.json")
...
class TestPush:
    ...
    def test_push_end_to_end_writes_cas_and_ref(self, integration_remote_ops_fn):
        ...
        with patch.object(remote_ops, "_local_dump_dict", return_value=local_manifest) as mock_dump:
            # Push the commit ref
            ref_path = remote_ops.push(Ref(f"commit:{commit_oid}"))

            # Verify _local_dump_dict was called
            mock_dump.assert_called_once()

            # Should return the ref path
            assert ref_path == f"commits/{commit_oid}.json"
    ...
    def test_push_ref_is_immutable(self, integration_remote_ops_fn):
        ...
        with patch.object(remote_ops, "_local_dump_dict", return_value=local_manifest):
            ...
            # Second push should fail with RefAlreadyExists
            with pytest.raises(RefAlreadyExists):
                remote_ops.push(Ref(f"commit:{commit_oid}"))
...
class TestPull:
    ...
    def test_pull_rejects_non_commit_root(self, remote_ops):
        ...
        ref_path = "invalid-root-ref.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)

        # Pull should fail with ValueError
        with pytest.raises(ValueError, match="Cannot pull non-commit root namespace: 'blob'"):
    ...
    def test_pull_downloads_missing_objects_only(self, integration_remote_ops_fn):
        ...
        remote_ops._remote_put_cas(commit_oid, commit_data)
        remote_ops._remote_put_cas(blob_oid, blob_data)
        ref_path = "commits/test-missing.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)
    ...
    def test_pull_verifies_sha256(self, remote_ops):
        ...
        ref_path = "commits/test-sha256.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)
    ...
    def test_pull_calls_txn_load_dict_with_local_manifest(self, remote_ops):
        ...
        remote_ops._remote_put_cas(manifest_id, manifest_bytes)
        remote_ops._remote_put_cas(commit_oid, commit_data)
        ref_path = "commits/test-load.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)
    ...
    def test_pull_writes_head_pointer(self, integration_remote_ops_fn):
        ...
        remote_ops._remote_put_cas(manifest_id, manifest_bytes)
        remote_ops._remote_put_cas(commit_oid, commit_data)
        ref_path = "commits/test-head.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)
...
class TestTask17PublicApiPolish:
    ...
    def test_push_raises_on_non_commit_root(self, remote_ops, db):
        """Test that push raises ValueError for non-commit root namespace."""
        ...
        with patch.object(remote_ops, "_local_dump_dict", return_value=local_manifest):
            with pytest.raises(ValueError, match="Cannot push non-commit root namespace: 'blob'"):
                remote_ops.push(Ref("commit:test"))

    def test_pull_raises_on_missing_cas_object(self, remote_ops):
        """Test that pull raises MissingCasObject when CAS object is missing."""
        ...
        ref_path = "commits/missing-manifest.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)

        # Pull should fail with MissingCasObject
        with pytest.raises(MissingCasObject, match=f"CAS object {manifest_id} not found"):
            remote_ops.pull(ref_path)
    ...
    def test_pull_raises_sha_mismatch_on_bad_cas_data(self, remote_ops):
        ...
        remote_ops._remote_put_cas(manifest_id, manifest_bytes)
        remote_ops._remote_put_cas(wrong_oid, blob_data)  # Wrong data for this OID
        ref_path = "commits/test-sha-mismatch.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)
...
class TestGcMark:
    ...
    def test_gc_mark_includes_manifest_and_closure_oids(self, remote_ops):
        ...
        ref_path = "commits/test-commit.json"
        remote_ops._remote_put_ref(ref_path, ref_bytes)
        ...
        # Test with multiple refs (tags, commits, cache)
        # Create a cache ref
        cache_ref_obj = {
            "kind": "ref",
            "schema": 0,
...
class TestList:
    ...
    def test_list_filters_prefix_correctly(self, remote_ops):
        ...
        remote_ops._remote_put_ref(
            "commits/commit1.json", json.dumps(commit_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )
        remote_ops._remote_put_ref(
            "tags/v1.0.0.json", json.dumps(tag_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )
        remote_ops._remote_put_ref(
        ...
        # List each prefix
        commits = remote_ops.list("commits")
        ...
        assert commits[0]["ref_path"] == "commits/commit1.json"
        assert tags[0]["ref_path"] == "tags/v1.0.0.json"
        assert cache[0]["ref_path"] == "cache/temp.json"

        # Should have correct targets
        assert commits[0]["target"] == commit_ref["target"]
        assert tags[0]["target"] == tag_ref["target"]
        assert cache[0]["target"] == cache_ref["target"]

    def test_list_skips_invalid_refs(self, remote_ops):
        """Test that list skips refs that can't be retrieved or decoded."""
        # Clear any existing refs from previous tests
        bucket = os.environ["DML_S3_BUCKET"]
        prefix = os.environ.get("DML_S3_PREFIX", "")
        refs_prefix = f"{prefix}/refs/" if prefix else "refs/"
        ...
        remote_ops._remote_put_ref(
            "commits/valid.json", json.dumps(valid_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )
        ...
        remote_ops.client.put_object(
            ...
            Key=remote_ops._ref_key("commits/invalid.json"),
            Body=invalid_json,
        )

        # List should only return the valid ref
        refs = remote_ops.list("commits")
        assert len(refs) == 1
        assert refs[0]["ref_path"] == "commits/valid.json"

    def test_list_returns_empty_list_when_no_refs(self, remote_ops):
        """Test that list returns empty list when no refs exist for prefix."""
        refs = remote_ops.list("nonexistent")
        assert refs == []
...
class TestPrune:
    ...
    def test_prune_deletes_only_expired_cache_refs(self, remote_ops):
        """Test that prune deletes only cache refs with expired expires_at."""
        # Create cache refs with different expiration times
        now = int(time.time())

        # Expired ref (expires in past)
        expired_ref = {
            "kind": "ref",
            "schema": 0,
            "target": "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
        ...
        valid_ref = {
            ...
            "meta": {
                "cache": {
                    "expires_at": now + 3600  # Future
                }
            },
        }

        # Ref without expires_at (should be ignored)
        no_expires_ref = {
            "kind": "ref",
            "schema": 0,
            "target": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        ...
        # Put the cache refs
        remote_ops._remote_put_ref(
            "cache/expired.json", json.dumps(expired_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )
        remote_ops._remote_put_ref(
            "cache/valid.json", json.dumps(valid_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )
        remote_ops._remote_put_ref(
            "cache/no-expires.json", json.dumps(no_expires_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )

        # Prune should delete only the expired ref
        deleted_count = remote_ops.prune()
        assert deleted_count == 1

        # Verify expired ref was deleted
        with pytest.raises(RemoteError):
            remote_ops._remote_get_ref("cache/expired.json")

        # Verify valid ref still exists
        retrieved_valid = remote_ops._remote_get_ref("cache/valid.json")
        assert json.loads(retrieved_valid) == valid_ref

        # Verify no-expires ref still exists
        retrieved_no_expires = remote_ops._remote_get_ref("cache/no-expires.json")
        assert json.loads(retrieved_no_expires) == no_expires_ref

    def test_prune_ignores_non_cache_or_missing_cache_block(self, remote_ops):
        """Test that prune ignores refs without cache metadata or missing cache block."""
        now = int(time.time())

        # Ref without meta block
        no_meta_ref = {
            "kind": "ref",
            "schema": 0,
            "target": "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
        ...
        # Ref with meta but no cache block
        no_cache_ref = {
            "kind": "ref",
            "schema": 0,
            "target": "fedcba0987654321fedcba0987654321fedcba0987654321fedcba0987654321",
        ...
        # Put the refs
        remote_ops._remote_put_ref(
            "cache/no-meta.json", json.dumps(no_meta_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )
        remote_ops._remote_put_ref(
            "cache/no-cache.json", json.dumps(no_cache_ref, separators=(",", ":"), sort_keys=True).encode("utf-8")
        )

        # Prune should not delete any refs (none have cache.expires_at)
        deleted_count = remote_ops.prune()
        assert deleted_count == 0

        # Verify refs still exist
        retrieved_no_meta = remote_ops._remote_get_ref("cache/no-meta.json")
        assert json.loads(retrieved_no_meta) == no_meta_ref

        retrieved_no_cache = remote_ops._remote_get_ref("cache/no-cache.json")
        assert json.loads(retrieved_no_cache) == no_cache_ref
...
class TestE2E:
    ...
    def test_e2e_push_pull_gc(self, aws_server, s3):
        ...
        push_db = FakeDb()
        push_remote_ops = RemoteOps(push_db, s3)

        # Step 2: Push to S3
        with patch.object(push_remote_ops, "_local_dump_dict", return_value=local_manifest):
            ref_path = push_remote_ops.push(Ref(f"commit:{commit_oid}"))
            assert ref_path == f"commits/{commit_oid}.json"
        ...
        pull_remote_ops = RemoteOps(pull_db, s3)
