...
from daggerml_cli.types import Argspec, ArgvNode, Commit, Dag, Datum, DmlRepoError, Head, KwargvNode, Tree
from tests.test__db import REF_ALPHABET, _gen_ref
from tests.test_types import _dag_strategy, _refs
...
class TestDagOps:
    @given(_dag_strategy(), st.data())
    @settings(max_examples=10)
    def test_list_and_describe(self, temp_bo, dag, data):
        refs = dag_ref, _, commit_ref, _ = setup(temp_bo, dag, data)
        ...
        assert desc["argspec"] == dag.argspec
        # Clean up
        with temp_bo._tx(readonly=False) as txn:
            for ref in set(refs):
                if ref:
    ...
    @given(_dag_strategy())
    def test_list_empty(self, temp_bo, dag):
        ops = DagOps(temp_bo._db)
        # No heads/commits
        assert ops.list() == []
    ...
    def test_get_argv_happy_path(self, temp_bo):
        ...
        # create a dag with argspec present and insert it directly
        with temp_bo._tx(readonly=False) as txn:
            ...
            dag_ref = txn.put(Dag(nodes=[argv_node_ref, kwargv_node_ref], names={}, result=None, argspec=argspec_ref))
        try:
            ops = DagOps(temp_bo._db)
            assert ops.get_argv(dag_ref) == argv_node_ref
        finally:
            with temp_bo._tx(readonly=False) as txn:
                for r in (dag_ref, argspec_ref, argv_node_ref, kwargv_node_ref, argv_datum_ref, kwargv_datum_ref):
                    if r:
                        txn.delete(r)

    def test_get_argv_missing_raises(self, temp_bo):
        """get_argv should raise when the DAG has no argv node."""
        # insert a dag without argspec
        with temp_bo._tx(readonly=False) as txn:
            dag_ref = txn.put(Dag(nodes=[], names={}, result=None, argspec=None))
        try:
            ops = DagOps(temp_bo._db)
