...
def resource_strategy(recursive=False):
    return st.builds(
        Resource,
        uri=st.text(alphabet=REF_ALPHABET + ":/", min_size=1, max_size=32),
        data=st.just(None) if recursive else st.one_of(st.none(), _refs("datum")),
...
def scalar_strategy(recursive=False):
    return st.one_of(
        resource_strategy(recursive=recursive),
        int_strategy(),
        float_strategy(),
        st.booleans(),
        st.text(),
...
class TestDbEnv:
    ...
    def test_open_requires_mapsize_for_large_db(self, tmp_path):
        ...
        refs = []
        with pytest.raises((DmlError, RuntimeError)):
            with _open() as env:
                with env.tx(readonly=False) as txn:
