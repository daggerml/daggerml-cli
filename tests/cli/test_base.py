...
from daggerml_cli._cli.base import (
    apply_help_config,
    build_error_payload,
    build_help_epilog,
    execute_command,
...
from tests.test_types import _refs
...
class TestGetRepoPath:
    ...
    @patch.dict(os.environ, {"DML_REPO_PATH": "/env/path"})
    def test_env_var_fallback(self):
        """Test DML_REPO_PATH env var when no --repo."""
...
class TestErrorNormalization:
    ...
    def test_normalize_error_message_repo_path_has_recovery_hint(self):
        ...
        assert "--repo" in msg or "DML_REPO_PATH" in msg
...
class TestExecuteCommand:
    ...
    @patch("daggerml_cli._cli.base.DmlOps.open")
    @patch("daggerml_cli._cli.base.get_repo_path")
    def test_successful_execution(self, mock_get_path, mock_open):
        ...
        with patch("daggerml_cli._cli.base.output_json") as mock_output:
            execute_command(args)

        mock_get_path.assert_called_once_with(None)
        mock_open.assert_called_once_with("/repo/path")
    ...
    @patch("daggerml_cli._cli.base.DmlOps.open")
    @patch("daggerml_cli._cli.base.get_repo_path")
    @patch("daggerml_cli._cli.base.output_error")
...
class TestTopLevelHelp:
    def test_dml_help_lists_operations(self):
        ...
        for op in ["commit", "head", "index", "cache", "dag", "node", "remote", "gc"]:
            assert op in out
