...
from daggerml_cli._cli.head import (
    execute_head_create,
    execute_head_delete,
    execute_head_list,
    setup_head_parser,
...
class TestExecuteHeadHandlers:
    ...
    def test_execute_head_create_with_from(self):
        ...
        # Mock the _tx context manager
        mock_txn = Mock()
        mock_head_obj = Mock()
        mock_head_obj.commit = Ref("commit:abc123")
        mock_txn.get.return_value = mock_head_obj
        mock_txn.__enter__ = Mock(return_value=mock_txn)
        mock_txn.__exit__ = Mock(return_value=None)
        mock_ops._tx.return_value.__enter__ = Mock(return_value=mock_txn)
        mock_ops._tx.return_value.__exit__ = Mock(return_value=None)
        ...
        assert result == {"ref": "head:feature", "commit": "commit:abc123"}
    ...
    def test_execute_head_create_without_from(self):
        ...
        mock_txn = Mock()
        mock_head_obj = Mock()
        mock_head_obj.commit = Ref("commit:def456")
        mock_txn.get.return_value = mock_head_obj
        mock_txn.__enter__ = Mock(return_value=mock_txn)
        mock_txn.__exit__ = Mock(return_value=None)
        mock_ops._tx.return_value.__enter__ = Mock(return_value=mock_txn)
        ...
        assert result == {"ref": "head:branch", "commit": "commit:def456"}
...
class TestHeadCLIIntegration:
    ...
    def test_head_create_without_from(self):
        ...
        assert result["ref"] == "head:feature"
