...
from daggerml_cli._cli.dag import (
...
from daggerml_cli.types import Argspec, ArgvNode, Dag, Datum, KwargvNode, LiteralNode
...
class TestDagCLIIntegration:
    ...
    def create_dag(self):
        """Create a finished DAG with argspec in the repo."""
        with self.base_ops._tx(readonly=False) as txn:
            datum_ref = txn.put(Datum(data=1))
        ...
        return dag_ref, node_ref, argv_node_ref, kwargv_node_ref, argspec_ref

    def test_dag_list(self):
        """Test dag list returns metadata."""
        dag_ref, node_ref, _, _, argspec_ref = self.create_dag()
        ...
        assert entry["argspec"] == f"Ref({argspec_ref.to})"
    ...
    def test_dag_describe(self):
        ...
        assert result["argspec"] == f"Ref({argspec_ref.to})"

    def test_dag_get_node(self):
        """Test dag get-node returns node reference."""
        dag_ref, node_ref, _, _, _ = self.create_dag()
    ...
    def test_dag_get_argv(self):
        ...
        dag_ref, _, argv_node_ref, _, _ = self.create_dag()
    ...
    def test_dag_get_kwargv(self):
        ...
        dag_ref, _, _, kwargv_node_ref, _ = self.create_dag()
