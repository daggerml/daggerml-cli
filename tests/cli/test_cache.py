...
from daggerml_cli._cli.cache import (
    execute_cache_clear,
...
from tests.test__db import _gen_ref
...
class TestSetupCacheParser:
    ...
    def test_parser_creation(self):
        ...
        args = parser.parse_args(["get", "argspec:def"])
        assert args.subcommand == "get"
        args = parser.parse_args(["delete", "argspec:def"])
    ...
    def test_get_parser_args(self):
        ...
        parser = ArgumentParser()
        setup_cache_parser(parser)
        args = parser.parse_args(["get", "argspec:def456"])
        assert args.subcommand == "get"
        assert args.argspec_ref == "argspec:def456"

    def test_delete_parser_args(self):
        """Test delete subcommand arguments."""
        parser = ArgumentParser()
        setup_cache_parser(parser)
        args = parser.parse_args(["delete", "argspec:def456"])
        assert args.subcommand == "delete"
        assert args.argspec_ref == "argspec:def456"
...
class TestExecuteCacheHandlers:
    ...
    def test_execute_cache_get_found(self):
        ...
        args = Namespace(argspec_ref="argspec:def456")
        result = execute_cache_get(mock_ops, args)

        mock_ops.get.assert_called_once_with(Ref("argspec:def456"))
    ...
    def test_execute_cache_delete(self):
        ...
        mock_ops.delete.assert_called_once_with(Ref("argspec:def456"))
        assert result is True

    def test_execute_cache_list(self):
        """Test execute_cache_list handler."""
        from daggerml_cli.types import Cache

        mock_ops = Mock()
        mock_cache_ref1 = Ref("cache:argspec1")
        dag_ref1 = Ref("dag:dag1")
        entry1 = Cache(dag_ref1)
        mock_cache_ref2 = Ref("cache:argspec2")
        dag_ref2 = Ref("dag:dag2")
        entry2 = Cache(dag_ref2)
        mock_ops.list.return_value = [(mock_cache_ref1, entry1), (mock_cache_ref2, entry2)]
        ...
        assert result == [
            ["cache:argspec1", {"dag": "dag:dag1"}],
            ["cache:argspec2", {"dag": "dag:dag2"}],
        ]
...
class TestCacheCLIIntegration:
    ...
    def setup_method(self):
        ...
        self.ops = CacheOps(self.dml_ops)
    ...
    def _put_argspec(self, datum_ref):
        with self.repo._tx(readonly=False) as txn:
    ...
    def _put_dag(self, argspec_ref=None):
        with self.repo._tx(readonly=False) as txn:
    ...
    def test_cache_put_stores_entry_and_returns_cache_ref(self):
        ...
        argspec_ref = self._put_argspec(datum_ref)
        dag_ref = self._put_dag(argspec_ref)
        ...
        stdout2, stderr2 = self.run_cli_command(["cache", "get", argspec_ref.to])
        assert not stderr2
        result2 = json.loads(stdout2.strip())
        assert result2 == dag_ref.to

    def test_cache_get_returns_cached_dag_ref_or_null(self):
        """Test dml cache get argspec:def456 returns cached dag ref string or null."""
        # First put something
        datum_ref = self._put_datum("test")
        argspec_ref = self._put_argspec(datum_ref)
        dag_ref = self._put_dag(argspec_ref)
        ...
        # Now get
        stdout, stderr = self.run_cli_command(["cache", "get", argspec_ref.to])
        assert not stderr
        result = json.loads(stdout.strip())
        assert result == dag_ref.to
        # Get non-existent
        argspec_ref2 = self._put_argspec(self._put_datum("other"))
        stdout2, stderr2 = self.run_cli_command(["cache", "get", argspec_ref2.to])
        assert not stderr2
        result2 = json.loads(stdout2.strip())
        assert result2 is None

    def test_cache_delete_removes_entry_and_returns_true_false(self):
        """Test dml cache delete argspec:def456 removes cache entry and returns true/false."""
        datum_ref = self._put_datum("test")
        argspec_ref = self._put_argspec(datum_ref)
        dag_ref = self._put_dag(argspec_ref)
        self.run_cli_command(["cache", "put", dag_ref.to])
        # Delete existing
        stdout, stderr = self.run_cli_command(["cache", "delete", argspec_ref.to])
        assert not stderr
        result = json.loads(stdout.strip())
        assert result is True
        # Delete again
        stdout2, stderr2 = self.run_cli_command(["cache", "delete", argspec_ref.to])
        assert not stderr2
        result2 = json.loads(stdout2.strip())
        assert result2 is False

    def test_cache_list_returns_json_array_of_pairs(self):
        """Test dml cache list --limit 10 returns JSON array of [argspec_ref, dag_ref] pairs."""
        # Add some entries
        entries = []
        for i in range(3):
            ...
            dag_ref = self._put_dag(argspec_ref)
            self.run_cli_command(["cache", "put", dag_ref.to])
            entries.append([f"cache:{argspec_ref.id()}", {"dag": dag_ref.to}])
        # List
