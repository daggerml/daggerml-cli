...
DmlValue *
dml_value_new_resource(const char *uri, size_t uri_len, DmlValue *data, const char *adapter, size_t adapter_len)
{
    DmlValue *result = dml_value_alloc(DML_VALUE_RESOURCE);
    if (result == NULL) {
        dml_value_free(data);
        return NULL;
    }
    result->as.resource.uri = (char *)malloc(uri_len);
    if (result->as.resource.uri == NULL) {
        dml_value_free(data);
        free(result);
        return NULL;
    }
    if (uri_len > 0 && uri != NULL) {
        memcpy(result->as.resource.uri, uri, uri_len);
    }
    result->as.resource.uri_len = uri_len;
    result->as.resource.data = data;
    result->as.resource.adapter = NULL;
    result->as.resource.adapter_len = 0;
    if (adapter != NULL) {
        result->as.resource.adapter = (char *)malloc(adapter_len);
        if (result->as.resource.adapter == NULL) {
            dml_value_free(data);
            free(result->as.resource.uri);
            free(result);
            return NULL;
        }
        if (adapter_len > 0) {
            memcpy(result->as.resource.adapter, adapter, adapter_len);
        }
        result->as.resource.adapter_len = adapter_len;
    }
    return result;
}
...
dml_value_free(DmlValue *value)
{
    ...
    case DML_VALUE_RESOURCE:
        free(value->as.resource.uri);
        dml_value_free(value->as.resource.data);
        free(value->as.resource.adapter);
        break;
    default:
        break;
    }
